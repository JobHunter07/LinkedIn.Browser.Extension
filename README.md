...
### 🦊 Firefox Add-ons
The extension is currently **under review** in the Mozilla Add-ons store.
In the meantime, you can manually build and install it using the steps below.
...
## 🧑‍💻 Development (Windows vs Linux)

- The npm scripts `dev:firefox` and `build:firefox` use `scripts/run-env.js` to set environment variables in a cross-platform way. On Windows it delegates to `cross-env`; on macOS/Linux it uses inline env assignments.
- `cross-env` is included as a dev dependency so Windows users don't need to set envs manually.

- Quick commands:
   - Install dependencies: `npm install`
   - Build for Chromium: `npm run build`
   - Build for Firefox (cross-platform): `npm run build:firefox`
   - Run dev server in Firefox mode: `npm run dev:firefox`

- Alternatives without the wrapper:
   - PowerShell: `$env:FIREFOX='true'; npm run build`
   - POSIX shells: `FIREFOX=true npm run build`
